...
if(!empty($data->pickup_latitude) && !empty($data->pickup_longitude)){
    ...
    if (isset($hospitals['features'])) {
        foreach ($hospitals['features'] as $key => $hospital) {
            
            $output_array["GTS"][$key]['name'] = $hospital['text'];
            $output_array["GTS"][$key]['address'] = $hospital['place_name'];
            $output_array["GTS"][$key]['lat'] = $hospital['geometry']['coordinates'][1];
            $output_array["GTS"][$key]['long'] = $hospital['geometry']['coordinates'][0];
